...
namespace Haley.Models {
    public static class QRY_WORKFLOW {
        ...
        public const string SELECT_LATEST_DEFINITION = $@"SELECT v.GUID, v.VERSION, v.DEFINITION, w.ID AS WORKFLOW_ID, w.CODE, w.NAME FROM WF_VERSION v INNER JOIN WORKFLOW w ON v.WORKFLOW = w.ID WHERE w.SOURCE = {SOURCE} AND w.CODE = {CODE} ORDER BY v.VERSION DESC LIMIT 1";
    }
