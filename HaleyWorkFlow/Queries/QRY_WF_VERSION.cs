...
namespace Haley.Models {
    public static class QRY_WF_VERSION {
        ...
        public const string SELECT_DEF_BY_WF_ID = $@"SSELECT v.*, w.CODE, w.NAME, w.SOURCE FROM WF_VERSION v INNER JOIN WORKFLOW w ON v.WORKFLOW = w.ID WHERE v.WORKFLOW = {WORKFLOW} ORDER BY v.VERSION DESC LIMIT 1";
        public const string SELECT_DEF_BY_GUID = $@"SSELECT v.*, w.CODE, w.NAME, w.SOURCE FROM WF_VERSION v INNER JOIN WORKFLOW w ON v.WORKFLOW = w.ID WHERE v.guid = {GUID}";
        public const string SELECT_DEF_BY_VER_ID = $@"SSELECT v.*, w.CODE, w.NAME, w.SOURCE FROM WF_VERSION v INNER JOIN WORKFLOW w ON v.WORKFLOW = w.ID WHERE v.id = {ID}";
